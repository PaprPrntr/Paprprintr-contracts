...
contract PAPRUSDCKudogePoolFarming is Ownable, ReentrancyGuard {
    ...
    require(msg.sender == rewardAdder, 'wut ar u tryin pajeet');
    _;
    }
    ...
    function addContractBalance(uint amount) public onlyRewardAdder {
        require(Token(trustedRewardTokenAddress).transferFrom(msg.sender, address(this), amount), "Cannot add balance!");
        contractBalance = contractBalance.add(amount);
    }

    function setRewardAdder(address _rewardAdder) public onlyOwner {
        require(_rewardAdder != address(0), 'U mad bruv');
        rewardAdder = _rewardAdder;
    }
    ...
    // function to allow owner to claim *other* modern ERC20 tokens sent to this contract
    function adderTransferRewards(address _to, uint _amount) public onlyRewardAdder {

        require(now > adminClaimableTime, "Admin cannot Transfer out Reward Tokens Yet!");
        require(Token(trustedRewardTokenAddress).transfer(_to, _amount), "Could not transfer out tokens!");
    }
